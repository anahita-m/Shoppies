...
export default class Footer extends Component{
    ...
    render(){
        const {darkMode, nominatedMovies, nominatedMovieTitles} = this.props;
        const config = {
            angle: 90,
            spread: 360,
            startVelocity: 40,
          ...
          const full = (nominatedMovies.length === 5 && !this.state.closed) ? true : false;
          if(this.state.closed && nominatedMovies.length === 4){
              this.resetClosed()
          }
